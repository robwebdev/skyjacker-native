...
GameSparks.prototype = {
  ...
  onWebSocketError: function(ev) {
    this.log(
      "WebSocket onError: Sorry, but there is some problem with your socket or the server is down"
    );
  ...
  onWebSocketMessage: function(message) {
    ...
    try {
      ...
      this.log(
        "An error ocurred while parsing the JSON Data: " +
          message +
          "; Error: " +
          e
      );
  ...
  handshake: function(result) {
    if (result["nonce"]) {
      ...
      if (this.options.onNonce) {
        ...
        hmac = CryptoJS.enc.Base64.stringify(
          CryptoJS.HmacSHA256(result["nonce"], this.options.secret)
        );
      }
...
GameSparks.prototype.acceptChallengeRequest = function(
  challengeInstanceId,
  message,
  onResponse
) {
...
GameSparks.prototype.analyticsRequest = function(
  data,
  end,
  key,
  start,
...
GameSparks.prototype.aroundMeLeaderboardRequest = function(
  count,
  friendIds,
...
GameSparks.prototype.authenticationRequest = function(
  password,
  userName,
  onResponse
) {
...
GameSparks.prototype.buyVirtualGoodsRequest = function(
  currencyType,
  quantity,
  shortCode,
  onResponse
...
GameSparks.prototype.changeUserDetailsRequest = function(
  displayName,
  onResponse
) {
  var request = {};
...
GameSparks.prototype.chatOnChallengeRequest = function(
  challengeInstanceId,
  message,
  onResponse
) {
...
GameSparks.prototype.consumeVirtualGoodRequest = function(
  quantity,
  shortCode,
  onResponse
) {
...
GameSparks.prototype.createChallengeRequest = function(
  accessType,
  challengeMessage,
  challengeShortCode,
  currency1Wager,
...
GameSparks.prototype.declineChallengeRequest = function(
  challengeInstanceId,
  message,
  onResponse
) {
...
GameSparks.prototype.deviceAuthenticationRequest = function(
  deviceId,
  deviceModel,
  deviceName,
  deviceOS,
...
GameSparks.prototype.facebookConnectRequest = function(
  accessToken,
  code,
  onResponse
) {
...
GameSparks.prototype.findChallengeRequest = function(
  accessType,
  count,
  offset,
  onResponse
...
GameSparks.prototype.getChallengeRequest = function(
  challengeInstanceId,
  message,
  onResponse
) {
...
GameSparks.prototype.getRunningTotalsRequest = function(
  friendIds,
  shortCode,
  onResponse
) {
...
GameSparks.prototype.googlePlayBuyGoodsRequest = function(
  currencyCode,
  signature,
  signedData,
  subUnitPrice,
...
GameSparks.prototype.iOSBuyGoodsRequest = function(
  currencyCode,
  receipt,
  sandbox,
  subUnitPrice,
...
GameSparks.prototype.joinChallengeRequest = function(
  challengeInstanceId,
  message,
  onResponse
) {
...
GameSparks.prototype.leaderboardDataRequest = function(
  challengeInstanceId,
  entryCount,
  friendIds,
  leaderboardShortCode,
...
GameSparks.prototype.listChallengeRequest = function(
  entryCount,
  offset,
  shortCode,
  state,
...
GameSparks.prototype.listMessageRequest = function(
  entryCount,
  offset,
  onResponse
) {
...
GameSparks.prototype.listMessageSummaryRequest = function(
  entryCount,
  offset,
  onResponse
) {
...
GameSparks.prototype.logChallengeEventRequest = function(
  challengeInstanceId,
  eventKey,
  onResponse
) {
...
GameSparks.prototype.pushRegistrationRequest = function(
  deviceOS,
  pushId,
  onResponse
) {
...
GameSparks.prototype.registrationRequest = function(
  displayName,
  password,
  userName,
  onResponse
...
GameSparks.prototype.sendFriendMessageRequest = function(
  friendIds,
  message,
  onResponse
) {
...
GameSparks.prototype.socialLeaderboardDataRequest = function(
  challengeInstanceId,
  entryCount,
  friendIds,
  leaderboardShortCode,
...
GameSparks.prototype.twitterConnectRequest = function(
  accessSecret,
  accessToken,
  onResponse
) {
...
GameSparks.prototype.windowsBuyGoodsRequest = function(
  currencyCode,
  receipt,
  subUnitPrice,
  onResponse
...
GameSparks.prototype.withdrawChallengeRequest = function(
  challengeInstanceId,
  message,
  onResponse
) {
...
function promisifyRequestMethods(gs) {
  Object.keys(GameSparks.prototype).forEach(key => {
    if (key.indexOf("Request") !== -1) {
      gs[`${key}Async`] = function() {
        return new Promise((resolve, reject) => {
          gs[key](...arguments, response => {
            if (response.error) {
              reject(response);
            } else {
              resolve(response);
...
export default function createGameSparksClient() {
  const gs = new GameSparks();
  promisifyRequestMethods(gs);
  return gs;
}
